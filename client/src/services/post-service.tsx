...
class PostService {
    ...
    createPost(title: string, user_id: number, content: string, image: string, created_at: Date) {
      return axios
        .post<{ post_id: number }>('/posts', { title: title, user_id: user_id, content: content, image: image, created_at: created_at })
    ...
    updatePost(post_id: number, title: string, content: string, image: string, created_at: Date, updated_at: Date) {
      return axios
        .put<{ post_id: number }>('/posts/' + post_id, { title: title, content: content, image: image, created_at: created_at, updated_at: updated_at })
        .then((response) => response.data.post_id);
    }
    ...
    addPostComment(post_id: number, user_id: number, content: string, created_at: Date) {
      return axios
        .post<{ postcomment_id: number }>('/posts/' + post_id + '/comments', { user_id, content, created_at })
